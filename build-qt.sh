...
SYSROOT_PATH=/opt/sysroot_debian11

#添加交叉编译工具链路径，根据前面交叉编译器安装的路径设置
CROSS_CHAIN_PREFIX=/opt/gcc-aarch64-none-linux-gnu-10.2/bin/aarch64-none-linux-gnu-
...
do_tar_package () {
     ...
     echo "COMPRESS_PACKAGE = ${COMPRESS_PACKAGE}"
     if [ ! -d "${PACKAGE_NAME}" ];then
